...
Detect = function()
	...
	if sanitizedfn ~= nil then
		...
		repeat
			local changed = false
			for _, pattern in pairs(ignoresuffixes) do
	...
	local file = buf.name  -- TODO: full path (but not used now anyway).
	local data = buf[1]    -- TODO: get more than just file line, vis gets 256 bytes.
